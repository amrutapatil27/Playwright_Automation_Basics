...
```
Error: locator.click: Error: strict mode violation: getByRole('link', { name: 'Open Page' }) resolved to 23 elements:
    1) <a class="tta-button is-ghost" href="./multiple_element_filter.html">…</a> aka getByRole('link', { name: 'Open page' }).first()
    2) <a href="./webtable.html" class="tta-button is-ghost">…</a> aka getByRole('link', { name: 'Open page' }).nth(1)
    3) <a class="tta-button is-ghost" href="./tables/practice.html">…</a> aka getByRole('link', { name: 'Open page' }).nth(2)
    4) <a class="tta-button is-ghost" href="./tables/webtable.html">…</a> aka getByRole('link', { name: 'Open page' }).nth(3)
    5) <a class="tta-button is-ghost" href="./tables/webtable1.html">…</a> aka getByRole('link', { name: 'Open page' }).nth(4)
    6) <a class="tta-button is-ghost" href="./tables/dropdowns.html">…</a> aka getByRole('link', { name: 'Open page' }).nth(5)
    7) <a class="tta-button is-ghost" href="./tables/select-boxes.html">…</a> aka locator('article:nth-child(5) > .index-card-foot > .tta-button').first()
    8) <a class="tta-button is-ghost" href="./tables/sortable.html">…</a> aka locator('article:nth-child(6) > .index-card-foot > .tta-button').first()
    9) <a class="tta-button is-ghost" href="./tables/scorecard.html">…</a> aka locator('article:nth-child(7) > .index-card-foot > .tta-button').first()
    10) <a href="./frames/index.html" class="tta-button is-ghost">…</a> aka locator('section:nth-child(9) > article > .index-card-foot > .tta-button').first()
    ...

Call log:
  - waiting for getByRole('link', { name: 'Open Page' })
...
> 5  |     await page.getByRole("link", {name:"Open Page"}).click();
     |                                                      ^ Error: locator.click: Error: strict mode violation: getByRole('link', { name: 'Open Page' }) resolved to 23 elements:
  6  |     // ✅ Best Practice (User-Centric)
  7  | const heading = page.getByRole('heading', { name: /QA.*Profile Form/i });
  8  | expect (heading).toHaveText("QA Profile Form");
  9  | 
  10 | });
  11 | 
```